    ...
    public function index()
    {
        ...
        if ($user->isAdmin()) {
            ...
            $popularBooks = Book::withCount('licenses')->withCount(['licenses as borrowed_count' => function($q) {
                $q->whereHas('loan');
            }])->orderByDesc('borrowed_count')->take(5)->get();
        ...
        $recentHistories = $user->loans()->with('license.book')->whereNotNull('returned_date')->latest('returned_date')->take(5)->get();

        $wishlist = $user->wishlists()->with('author')->take(5)->get();

        return view('dashboards.user', compact('currentRead', 'stats', 'recentHistories', 'wishlist'));
    }
}